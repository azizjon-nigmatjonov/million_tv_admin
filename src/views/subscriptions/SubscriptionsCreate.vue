<template>
  <div>
    ...
    <div v-if="edit">
      <a-card class="card-back" :title="$t('updateSub')">
        <template slot="extra">
            <a-button @click="$router.go(-1)" type="link" icon="arrow-left" style="margin-right: 10px"></a-button>
        </template>
        <a-row>
    ...
    <div v-else>
      <a-card class="card-back" :title="$t('fillIn')">
        <template slot="extra">
          <router-link :to="{ path: `/subscription/category/update/${this.$route.query.category_id}` }">
