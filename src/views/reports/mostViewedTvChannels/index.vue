<template>
  <a-card :title="$t('Самые просматриваемые телеканалы')">
...
export default {
  ...
  methods: {
    downloadExcel() {
      this.$store.dispatch('getMostViewedTvChannelsExcel', this.params).then(({ url }) => {
        window.open(url, '_blank')
      })
    },
    getInfo(params = {}) {
      this.$store
        .dispatch('getMostViewedTvChannels', params)
        .then((res) => {
          this.dataTable = res.total.map((el, i) => ({ ...el, index: i + (this.number ? this.number - 10 : 1) }))
        })
        .catch((error) => {
